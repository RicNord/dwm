...
/* Like deck but with vertical gaps */
void
zendeck(Monitor *m)
{
	unsigned int i, n;
	...
	int mrest, srest;
	Client *c;
    int gappovzen = (int)(m->mx * gappfaczen); // gap pixels from factor

	getgaps(m, &oh, &ov, &ih, &iv, &n);
	if (n == 0)
		return;

	sx = mx = m->wx + gappovzen;
	sy = my = m->wy + oh;
	sh = mh = m->wh - 2*oh - ih * (MIN(n, m->nmaster) - 1);
	sw = mw = m->ww - 2*gappovzen;
